...
4. Marcas: LOCKED / PROVISIONAL / OPEN. Lo OPEN no se implementa como si estuviera cerrado. Ver `docs/decisiones/DECISIONS.md`.

## Qué no hacer

- No implementar el juego completo de una vez. Sigue `docs/roadmap/ROADMAP.md`.
- KISS / DRY: no añadas sistemas “por si acaso”; no copies reglas entre docs ni entre engine y UI.
...
| Área | Skill | Docs |
| --- | --- | --- |
| Reglas de juego, pacing, roles | `theclutch-game-design` | `docs/rules/GAME_DESIGN.md`, models, CAREER_SYSTEM |
| Fórmulas, RNG, batches | `theclutch-simulation` | `docs/rules/SIMULATION.md` |
| API, DB, auth, ranking | `theclutch-backend` | `docs/architecture/ARCHITECTURE.md`, DAILY_MODE |
| UI mobile-first | `theclutch-frontend` | GAME_DESIGN §12, ARCHITECTURE §6 |
| Eventos y copy | `theclutch-events` | `docs/rules/EVENT_SYSTEM.md` |
| Tests y exploits | `theclutch-qa` | ROADMAP |
