...
| `firstName`, `lastName` | sí | generado por seed + nacionalidad |
| `nationality` | sí | código ISO-like ficticio o real de país (países sí; clubs no) |
| `age` | sí | entero, avanza 1 por temporada |
| `heightCm` | sí | generado por posición + ruido |
| `position` | sí | `PG \| SG \| SF \| PF \| C` |
| `secondaryPosition` | opcional | **OPEN** [D-08](DECISIONS.md) |
| `archetype` | sí | ver §4 |
...
Fórmula exacta: **OPEN** para números, **LOCKED** como principio. La implementa Simulation/Balance y se documenta en [SIMULATION.md](SIMULATION.md) cuando se cierre.
...
No todos se muestran. UI de MVP: overall + 4–6 atributos "signature" del arquetipo. El resto existe para la sim.
...
Los umbrales numéricos los cierra Simulation/Balance. Content no inventa badges sin pasar por Game Design.

Caps: **PROVISIONAL** máximo 5 badges activos en MVP para no inflar al All-Around.

## 6. Estado psicológico y físico

| Campo | Visible | Rango | Notas |
| --- | --- | --- | --- |
| `morale` | sí (simplificado) | 0–100 | minutos, resultados, eventos |
...
La UI puede mostrar un adjetivo ("Competidor", "Profesional") derivado. No mostrar los 5 números.
...
Contrato (ver [CAREER_SYSTEM.md](CAREER_SYSTEM.md)): salario, años, rol esperado, opciones, nivel de equipo, competición.
...
Edad de inicio **PROVISIONAL**: 16–19. Ruta Europa/NCAA es decisión temprana, no necesariamente pre-generada. Ver [D-04](DECISIONS.md).
...
- momentos (lista corta de flags: "olympic_gold", "undrafted_mvp", ...).

Fórmula de Legacy Score: [CAREER_SYSTEM.md](CAREER_SYSTEM.md).
