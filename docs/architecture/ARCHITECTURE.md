...
- La pre-alpha emite telemetría first-party de embudo (`landing_view`, inicio por modo, banda de tiempo hasta el primer recap, carrera terminada, replay y feedback). El tiempo no se envía exacto: solo `<5`, `5–10`, `10–20` o `20+ min`, codificado en una lista cerrada de eventos. **Sin cookies, cuenta, IP persistida, user-agent, nombre de jugador, seeds, decisiones ni identificador persistente.** El endpoint persiste solo `{event, viewport, createdAt}` en PostgreSQL. Si la base no está disponible, degrada a log estructurado sin romper el juego.
